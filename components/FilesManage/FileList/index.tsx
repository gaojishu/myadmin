...
export default function FilesList({
    ...
    const fileType = store.getState().commonEnumsState.filesTypeEnum;
    const [filesPageData, setFilesPageData] = useState<Pageable<FilesData>>();
    // const [selectFiles, setSelectFiles] = useState<FilesRecord[]>([]);

    const handlerSelect = (item: FilesData) => {
        // 检查 record 是否已在数组中
            ...
            <Affix offsetBottom={0}>
                <div className='bg-white p-4 flex flex-y-center border-t border-gray-300'>
                    <Pagination
                        defaultCurrent={1}
                        pageSize={9}
                        total={filesPageData?.page.totalElements}
